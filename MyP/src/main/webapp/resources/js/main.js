...
*/
    ...
    // 챗봇
    //.off() 이벤트 버블링 방지
    $(".guideButton").off().click(function () {
        var hasOn = $(".guide").hasClass("on")
        //console.log(hasOn);
        if (hasOn) {
            $(".guide").removeClass("on");
            $(".guideButton").attr("src", "resources/icons/chatrobot.svg");

        } else {
            $(".guide").addClass("on");
            $(".guideButton").attr("src", "resources/icons/chat_close.svg");
        }
    });
