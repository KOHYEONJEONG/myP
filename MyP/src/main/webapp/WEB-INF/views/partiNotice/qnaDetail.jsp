...
<body>
	...
	<form action="#" id="action2Form" method="post">
		<input type="hidden" name="gbn" value="delete" /> <input
			type="hidden" name="no" value="${data.QNA_NUM}" />
		<!-- 전 화면에서 넘어온 페이지 정보 -->
		<input type="hidden" name="page" id="page" value="${param.page}" />
		<!-- 전 화면에서 넘어온 검색 정보 -->
		<input type="hidden" name="searchGbn" id="searchGbn"
			value="${param.searchGbn}" /> <input type="hidden" name="searchTxt"
				...
				<div class="detail_wrap">
					...
					<c:choose>
						...
						<c:when test="${sMemNo eq data.MEM_NUM  and data.PRIVATE == 0}">
							<div class="notice">
								<div class="notice_left">
									<span class="i"></span> <span>${data.TITLE}</span>
								</div>
						...
						<c:when test="${data.PRIVATE == 0 }">
							...
							<div class="notice">
								<div class="notice_left">
									<span class="i"></span> <span>비공개</span>
								</div>
								<div class="notice_right">
									<div>비공개</div>
									<div>비공개</div>
